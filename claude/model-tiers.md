...
| fallback | 5.6系が stall・未対応・不調のとき | `gpt-5.5`（前世代） | 明示指定 |
...
- 例外注記: claudex のモデル名ショートカット（sol/terra/luna/5.5）は `~/.zshrc` の関数側に実装がある — GPT 世代交代時は本表とあわせて claudex 関数も更新する
...
## モデル交代ランブック
1. 本表を書き換える（他のファイルは触らない＝正典を分散させない）
