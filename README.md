...
- モデル切替: `/model` ピッカー（**Claude プリセットを隠し GPT だけ表示**・From gateway ラベル）から選ぶ、または `/model gpt-5.6-terra` 直打ち。claudex 関数が discovery キャッシュの実IDから allowlist を毎起動自動生成する（自己修復）
- 仕組み: 公式 `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY`（v2.1.129+）でプロキシの `/v1/models` をピッカーへ＋`availableModels`（実ID・トップレベル）で Claude プリセットを非表示。**Default 行の1つだけは本体仕様で消せない**（中身は sol に解決＝ラベルのみ静的表示）。すべて claudex 関数内のみ＝素の `claude` は不変
- 任意Tip: ピッカーを絞るには cliproxyapi.conf の `oauth-excluded-models:`→`codex:` で不要モデルを列挙（除外しても名前直指定では使える）
