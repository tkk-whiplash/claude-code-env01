...
- モデル切替: セッション内は `/model gpt-5.6-terra` の直打ち、または `/model` ピッカーを**↑スクロール**（プロキシのモデルが「From gateway」ラベルで並ぶ＝公式の `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY`、v2.1.129+。claudex 関数内のみで有効化）
- 任意Tip: ピッカーを実用モデルだけにするには cliproxyapi.conf 末尾に `oauth-excluded-models:`→`codex:` で image 系等を列挙（除外しても名前直指定では使える）
- 任意Tip2（誤選択の無害化）: ピッカーの Claude プリセット行は**どの設定でも消せない**（実測）。代わりに cliproxyapi.conf の `oauth-model-alias:`→`codex:` で `claude-sonnet-5`→terra 等の読み替え（`fork: true`）を張ると、**プリセットを選んでも GPT 三層へ正しくルーティング**され「選んではいけない行」が消滅する
